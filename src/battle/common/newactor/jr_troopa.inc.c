...
extern EvtScript N(EVS_Attack_Double_Leap);
...
enum N(ActorVars) {
    AVAR_Phase                  = 0,
    AVAL_FirstPhaseCutscene     = 0,
    AVAL_FirstPhase             = 1,
    AVAL_SecondPhaseCutscene    = 2,
    AVAL_SecondPhase            = 3,
    AVAR_ShowedNewPower         = 1,
    AVAR_HitReact_State         = 2,
    AVAL_HitReact_None          = 0,
    AVAL_HitReact_Ready         = 1,
    AVAL_HitReact_Done          = 2,
    AVAR_Speaked                = 3,
    AVAL_SpeakedFirstPhase      = 0,
    AVAL_SpeakedSecondPhase     = 1,
};
...
EvtScript N(EVS_Init) = {
    ...
    Call(SetActorVar, ACTOR_SELF, AVAR_Phase, AVAL_FirstPhaseCutscene)
...
EvtScript N(EVS_HandleEvent) = {
    ...
    Switch(LVar0)
        ...
        CaseEq(EVENT_SHOCK_HIT)
            ...
            Call(GetActorVar, ACTOR_SELF, AVAR_Phase, LVarA)
            IfEq(LVarA, AVAL_SecondPhase)
    ...
    IfEq(LVarA, AVAL_SecondPhase)
        ...
        IfEq(LVar0, AVAL_HitReact_Ready)
            ...
            IfNotFlag(LVar0, STATUS_FLAGS_DOJO)
                ...
                Call(ActorSpeak, MSG_Misc_Test, ACTOR_SELF, PRT_MAIN, ANIM_JrTroopa_Dizzy, ANIM_JrTroopa_Dizzy)
...
#define LBL_END_TURN 0
#define LBL_PHASE_1 1
#define LBL_PHASE_2 2
EvtScript N(EVS_TakeTurn) = {
    Call(UseIdleAnimation, ACTOR_SELF, FALSE)
    Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_DISABLE)
    Call(GetActorVar, ACTOR_SELF, AVAR_Phase, LVar0)
    Switch(LVar0)
        CaseEq(AVAL_FirstPhaseCutscene)
            Call(SetActorVar, ACTOR_SELF, AVAR_Phase, AVAL_FirstPhase)
            Goto(LBL_PHASE_1)
            Return
        CaseEq(AVAL_FirstPhase)
            Label(LBL_PHASE_1)
            Call(GetActorHP, ACTOR_SELF, LVar7, LVar8)
            IfLe(LVar7, 10)
                Call(SetActorVar, ACTOR_SELF, AVAR_Phase, AVAL_SecondPhaseCutscene)
            Else
                ExecWait(N(EVS_Attack_Leap))
                Goto(LBL_END_TURN)
            EndIf
            Return
        CaseEq(AVAL_SecondPhaseCutscene)
            Call(SetActorVar, ACTOR_SELF, AVAR_Phase, AVAL_SecondPhase)
            Goto(LBL_PHASE_2)
            Return
        CaseEq(AVAL_SecondPhase)
            Label(LBL_PHASE_2)
            ExecWait(N(EVS_Attack_Double_Leap))
            Goto(LBL_END_TURN)
    EndSwitch
    Label(LBL_END_TURN)
    Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_DISABLE)
...
EvtScript N(EVS_Attack_Double_Leap) = {
    Call(UseIdleAnimation, ACTOR_SELF, FALSE)
    Call(EnableIdleScript, ACTOR_SELF, IDLE_SCRIPT_DISABLE)
    Call(PlaySoundAtActor, ACTOR_SELF, SOUND_OPEN_SHELL)
    Call(SetAnimation, ACTOR_SELF, PRT_MAIN, ANIM_JrTroopa_Idle)
    ...
    Loop(2)
        ...
        Call(JumpToGoal, ACTOR_SELF, 15, FALSE, TRUE, FALSE)
        Wait(2)
        Call(EnemyDamageTarget, ACTOR_SELF, LVar0, 0, 0, 0, DMG_DOUBLE_LEAP, BS_FLAGS1_TRIGGER_EVENTS)
    EndLoop
    Switch(LVar0)
        CaseDefault
            Call(UseBattleCamPreset, BTL_CAM_DEFAULT)
        ...
        CaseEq(PHASE_PLAYER_BEGIN)
            ...
            Switch(LVarA)
                CaseEq(AVAL_FirstPhaseCutscene)
                    ...
                    ExecWait(N(EVS_Cam_FocusOnJrTroopa))
                    Wait(10)
                    Call(ActorSpeak, MSG_Misc_Test, ACTOR_SELF, PRT_MAIN, ANIM_JrTroopa_PointTalk, ANIM_JrTroopa_PointTapFoot)
                    Return
                CaseEq(AVAL_SecondPhaseCutscene)
                    Call(SetActorVar, ACTOR_SELF, AVAR_Phase, AVAL_SecondPhase)
                    ...
                    IfFalse(LVar0)
                        ...
                        Call(ActorSpeak, MSG_Misc_Test, ACTOR_SELF, PRT_MAIN, ANIM_JrTroopa_EggTalk, ANIM_JrTroopa_EggIdle)
                        Wait(10)
                        ExecWait(N(EVS_Cam_ResetFocus))
                        Call(SetEnemyTargetOffset, ACTOR_SELF, PRT_MAIN, -6, 18)
                        Call(SetActorType, ACTOR_SELF, ACTOR_TYPE_JR_TROOPA_2)
                        Call(SetDefenseTable, ACTOR_SELF, PRT_MAIN, Ref(N(EggDefense)))
                    EndIf
                    Return
            EndSwitch
